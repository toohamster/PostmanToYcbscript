...
function (JsonSchema,AScript) {
	...
	Ken.convert = function(postman, version){
		var validate = null;
		version = version || Ken.probeVersion(postman);
		switch (version)
		{
			case 'v1':
				validate = JsonSchema.validate(postman, schemaV1);
				break;
			case 'v2':
				validate = JsonSchema.validate(postman, schemaV2);
				break;
			default:
				throw new TypeError('无效参数 version');
				break;
		}
		...
		if ( validate )
		{
			if ( 'v1' == version )
			{
				return Ken.v1(postman);
			}
			if ( 'v2' == version )
			{
				return Ken.v2(postman);
			}
		}
