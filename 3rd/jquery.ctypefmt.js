...
(function($) {
	...
	var fmtHTML = function(str)
	{
        ...
        if (str && str.charAt(0) === '<')
        {
            return style_html(str, tabsize, tabchar, 120);
        }
        else
        {
            return js_beautify(str, tabsize, tabchar);
        }
	...
	$.cTypeParse = function(str, ctype) {
		...
		if ('json') return $.parseJSON(str);
		if ('xml') return $.parseXML(str);
		if ('html') return $.parseHTML(str);
		return str;
	}
	...
	$.cTypeFmt = function(str, ctype) {
		...
		ctype = ctype.toLowerCase();
		console.log(ctype);
		if ('json') return fmtJSON(str);
		if ('xml') return fmtXML(str);
		if ('html') return fmtHTML(str);
		return str;
	}
